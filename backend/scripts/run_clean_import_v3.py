...
def main():
    ...
    stats = {"buses_updated": 0, "buses_created": 0, "itv": 0, "seguros": 0}

    # Recorrer exactamente las filas de datos de los 1865 buses (filas 7 a 1875)
    for row_idx in range(DATA_START, 1875):
        ...
        # 3. ITV Bus — Upsert con ON CONFLICT (id_bus)
        if venc_itv:
            ...
            try:
                ...
                print(f"Error ITV row {row_idx}: {e_itv}")
    ...
    print(f"Final DB Count in itv_bus: {cur.fetchone()[0]}")
    cur.execute(f"SELECT count(*) FROM {SCHEMA}.seguros_bus;")
    print(f"Final DB Count in seguros_bus: {cur.fetchone()[0]}")
