...
async def _get_or_create_marca(db: AsyncSession, cache: dict, nombre: Optional[str]) -> Optional[int]:
    ...
    if key in cache:
        return cache[key]
    res = await db.execute(select(Marca.id_marca).where(func_upper_eq(Marca.nombre, nombre)))
    row = res.scalar_one_or_none()
    if row:
        cache[key] = row
        return row
    # fallback exact + create
    res = await db.execute(select(Marca).where(Marca.nombre == nombre.strip()[:100]))
...
async def _get_or_create_marca_carr(db: AsyncSession, cache: dict, nombre: Optional[str]) -> Optional[int]:
    ...
    if key in cache:
        return cache[key]
    res = await db.execute(select(MarcaCarroceria).where(MarcaCarroceria.nombre == nombre.strip()[:100]))
    m = res.scalar_one_or_none()
    if m:
    ...
    res = await db.execute(
        select(MarcaCarroceria).where(func_upper_eq(MarcaCarroceria.nombre, nombre))
    )
    m = res.scalar_one_or_none()
    if m:
        cache[key] = m.id_marca_carroceria
...
async def _get_or_create_tipo_carr(db: AsyncSession, cache: dict, desc: Optional[str]) -> Optional[int]:
    ...
    res = await db.execute(select(TipoCarroceria).where(TipoCarroceria.descripcion == d))
    m = res.scalar_one_or_none()
    if not m:
        res = await db.execute(select(TipoCarroceria).where(func_upper_eq(TipoCarroceria.descripcion, d)))
        m = res.scalar_one_or_none()
    if m:
        cache[key] = m.id_tipo
        return m.id_tipo
...
async def _resolve_tipo_servicio(db: AsyncSession, cache: dict, nombre: Optional[str]) -> Optional[int]:
    ...
    if nombre in cache:
        return cache[nombre]
    res = await db.execute(select(TipoServicio).where(TipoServicio.nombre == nombre))
    m = res.scalar_one_or_none()
    if m:
...
async def apply_import(
    ...
    *,
    ...
    for r in rows:
        try:
            ...
            if bus:
                ...
                if r.rua and (not bus.rua or bus.rua.upper() == "N/D"):
                    bus.rua = r.rua
                if r.chassis and bus.numero_chassis != r.chassis:
                    # no sobrescribir chassis si ya hay uno distinto con riesgo de unique
                    pass
                if sincronizar_estado and (bus.estado_bus or "").upper() != "ACTIVO":
                    bus.estado_bus = "ACTIVO"
                    result.buses_activados += 1
                result.buses_actualizados += 1
            elif crear_faltantes:
                rua_val = r.rua or f"CHASSIS_{r.chassis}"
                chassis_val = r.chassis or f"RUA_{r.rua}"
                bus = Bus(
                    numero_orden=r.numero_orden,
                    id_marca=id_marca,
                    año=r.anio or 2000,
                    numero_chassis=chassis_val[:50],
                    rua=rua_val[:20],
            ...
            matched_ids.add(bus.id_bus)

            # ITV vigente
            if r.vencimiento_itv:
                itv_q = await db.execute(
                    select(ItvBus).where(ItvBus.id_bus == bus.id_bus, ItvBus.es_vigente.is_(True))
                )
                current = itv_q.scalar_one_or_none()
            ...
            msg = f"Fila {r.row_num} RUA={r.rua}: {exc}"
            result.errores.append(msg)
            if len(result.errores) > 50:
                break

    if sincronizar_estado:
        for b in buses:
            if b.id_bus not in matched_ids and (b.estado_bus or "").upper() == "ACTIVO":
    ...
    await db.commit()
    _ = (hoja, fecha_corte, usuario)  # reserved for auditoría futura
    return result
