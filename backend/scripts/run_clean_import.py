...
def run_import():
    ...
    print("Clearing old itv_bus, seguros_bus, historial_itv...")
    ...
    print("  New Buses created:", bus_count)
    print("  ITV rows inserted:", itv_count)
    print("  Seguros rows inserted:", seg_count)

    cur.execute(f"SELECT count(*) FROM {SCHEMA}.itv_bus;")
    print("  Final DB count itv_bus:", cur.fetchone()[0])
    cur.execute(f"SELECT count(*) FROM {SCHEMA}.seguros_bus;")
    print("  Final DB count seguros_bus:", cur.fetchone()[0])
